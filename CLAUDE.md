...
**Resuelto (18/09/2026):** la skill de proyecto `wireless-network-analyst` (`.claude/skills/wireless-network-analyst/SKILL.md`) ya integra estas 3 tools, `perfilar_cliente_pcap` y `escanear_a_pcap`, y refleja la preferencia `iw` sobre `airmon` en sus flujos.
...
- **Playbook de health checks/troubleshooting en runtime** (las 27 tools): skill de proyecto `wireless-network-analyst` — health check diario, revisión semanal (lunes), troubleshooting de cliente, auditoría WPA2/WPA3, detección de ataques y análisis forense de PCAP.
